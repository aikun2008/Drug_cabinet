...
// 减少延迟时间以提高通信速度
#define RC522_DELAY()  do { \
    __NOP(); __NOP(); __NOP(); \
} while(0)

//static SPI_HandleTypeDef hspi_rc522;
...
void MFRC522_Init(void)
{
    ...
    /* 配置 SDA 引脚 */
    GPIO_InitStruct.Pin = MFRC522_GPIO_SDA_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(MFRC522_GPIO_SDA_PORT, &GPIO_InitStruct);

    /* 配置 SCK 引脚 */
    GPIO_InitStruct.Pin = MFRC522_GPIO_SCK_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(MFRC522_GPIO_SCK_PORT, &GPIO_InitStruct);

    /* 配置 MOSI 引脚 */
    GPIO_InitStruct.Pin = MFRC522_GPIO_MOSI_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(MFRC522_GPIO_MOSI_PORT, &GPIO_InitStruct);

    /* 配置 MISO 引脚 */
    GPIO_InitStruct.Pin = MFRC522_GPIO_MISO_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    HAL_GPIO_Init(MFRC522_GPIO_MISO_PORT, &GPIO_InitStruct);

    /* 配置 RST 引脚 */
    GPIO_InitStruct.Pin = MFRC522_GPIO_RST_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(MFRC522_GPIO_RST_PORT, &GPIO_InitStruct);
...
char MFRC522_Reset(void) 
{
    ...
    int timeout = 500; // 减少超时时间
    while((Read_MFRC522(CommandReg) & 0x10) && timeout > 0) {
        HAL_Delay(1);
        timeout--;
    }
    ...
    Write_MFRC522(ModeReg,0x3D);            //和Mifare通讯，CRC初始值0x6363
                 ...
                 unsigned int  *pOutLenBit)
{
    ...
    Write_MFRC522(ComIEnReg,irqEn|0x80); //PCD_TRANSCEIVE模式下允许中断
    ClearBitMask(ComIrqReg,0x80);        //IRQ引脚不接收中断信号
    Write_MFRC522(CommandReg,PCD_IDLE);  //取消当前命令
    SetBitMask(FIFOLevelReg,0x80);       //清空FIFO缓冲区内容和ErrReg寄存器的BufferOvfl标志
   
    for (i=0; i<InLenByte; i++)
    {   
        Write_MFRC522(FIFODataReg, pInData[i]);    //将数据存储到FIFO
    }
    Write_MFRC522(CommandReg, Command);   //执行FIFO命令
   
    
    if (Command == PCD_TRANSCEIVE)
    {    
        SetBitMask(BitFramingReg,0x80);  //启动数据发送，StartSend=1
    }
   
    n = Read_MFRC522(ComIrqReg);
    i = 1500;//等待时间根据实际工作频率调整，这里M1卡通信等待时间25ms
    do 
    {
         n = Read_MFRC522(ComIrqReg);
         i--;
...
//函数名：CalulateCRC
//描述：用MF522计算CRC
//参数：pIndata--要计算CRC的数据指针；len--数据长度；pOutData--计算出的CRC结果
//返回值：无
/////////////////////////////////////////////////////////////////////
